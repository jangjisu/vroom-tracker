...
export function createRouteRestStopRequest({ fetchImpl = fetch, onState = () => {} } = {}) {
    ...
    async function load(originLatitude, originLongitude, destinationQuery) {
        ...
        try {
            const url = `${ROUTE_REST_STOPS_ENDPOINT}?originLat=${originLatitude}`
                + `&originLng=${originLongitude}`
                + `&destinationQuery=${encodeURIComponent(query)}`;
